...
BEGIN
insert into xtrn(xtrn,xtypetrn,zid,zactive,xnum,ztime) values('PR--','Promo Code',@zid,'1',1000,CURRENT_TIMESTAMP)
SELECT @lastnum = xnum from xtrn where zid=@zid AND xtypetrn=@typetrn and xtrn=@trn
END
IF @lastnum=0 AND @typetrn='MR Code'
BEGIN
insert into xtrn(xtrn,xtypetrn,zid,zactive,xnum,ztime) values('MR--','MR Code',@zid,'1',1000,CURRENT_TIMESTAMP)
SELECT @lastnum = xnum from xtrn where zid=@zid AND xtypetrn=@typetrn and xtrn=@trn
END
IF @lastnum=0 AND @typetrn='Customer Code'
BEGIN
insert into xtrn(xtrn,xtypetrn,zid,zactive,xnum,ztime) values('CUS-','Customer Code',@zid,'1',1000,CURRENT_TIMESTAMP)
SELECT @lastnum = xnum from xtrn where zid=@zid AND xtypetrn=@typetrn and xtrn=@trn
END
